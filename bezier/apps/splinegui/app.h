...
void update_curve_shape(Gui_Spline& spline, vector<vec3f>& bad_samples,
    vector<vec3f>& good_samples, int curve_id, const bezier_mesh& mesh,
    const bezier_params& params, vector<float>& bad_t,
    vector<mesh_point>& points, const bool jumps = false,
...
struct App {
  ...
  vector<vec3f>         good_samples        = {};
  Added_Points*         good_samples_shape  = {};
  vector<vec3f>         bad_samples         = {};
  Added_Points*         bad_samples_shape   = {};
...
inline void update_camera_info(App& app, const gui_input& input) {
  ...
  app.matrices.projection = perspective_mat(
      camera_yfov, camera.aspect, app._shade_params.near, app._shade_params.far);
...
//     const bool use_vector_heat, vector<float>& bad_t, vector<int>& jumps) {
//   return bezier_karcher_final(mesh.solver, mesh.triangles, mesh.positions,
//       mesh.adjacencies, mesh.normals, mesh.v2t, mesh.angles,
//       mesh.total_angles, mesh.Grad, points, number_of_subdivision, jumps,
//       *mesh.flipout, bad_t, use_vector_heat);
// }