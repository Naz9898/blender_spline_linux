...
void set_common_uniforms(const App& app, const ogl_program* program) {
  ...
  set_uniform(program, "gamma", app._shade_params.gamma);
  set_uniform(program, "exposure", app._shade_params.exposure);
...
void draw_scene(const App& app, const vec4i& viewport) {
  ...
  draw_scene(app.scene, app.camera, viewport, app._shade_params);
  ...
  auto projection = perspective_mat(
      camera_yfov, camera_aspect, app._shade_params.near, app._shade_params.far);
  ...
  /*
    if (key == gui_key('P')) {
      ...
      if (app.quadric_curve) {
        ...
        if (app._bezier_params.algorithm ==
                spline_algorithm::de_casteljau_uniform ||
            app._bezier_params.algorithm ==
                spline_algorithm::de_casteljau_adaptive)
          std::tie(left, right) = insert_point(
              app.mesh, curve.tree, app.input().parameter_t);
        else
          std::tie(left, right) = insert_point_spline(app.mesh,
              ...
              app.input().parameter_t, app._bezier_params);
...
bool process_user_input(App& app, const gui_input& input) {
  ...
  if (is_pressing(input.mouse_left) && !editing) {
    ...
    if (!app.weighted_average) {
      ...
      add_points_shape(app, app.control_points, 0.003, {0, 0, 1});
      return true;
    }
  }
  ...
  if (is_down(input.mouse_left) && length(drag) > 0.05) {
    ...
    if (app.input().rotating) {
      for (auto& spline : app.splines()) {
        ...
        auto params = app._bezier_params;
...
void update_app(App& app, const gui_input& input) {
  ...
  auto f = [&](int i) {
    ...
    update_curve_shape(spline, curve_id, app.mesh, app._bezier_params);
  };
  ...
  if (tasks.size()) {
    ...
    for (auto& spline : app.splines()) {
      for (auto& k : spline.curves_to_update) {
        ...
        if (app.enable_jumps)
          update_path_shape(curve.shape, app.mesh, curve.positions,
...
void draw(const gui_input& input, void* data) {
  ...
  if (draw_button(widget, "Save test")) {
    for(int i = 0; app.mesh.positions.size(); i++){
      std::cout << app.mesh.positions[i].x << " " << app.mesh.positions[i].y << " " << app.mesh.positions[i].z << "\n";
    }
  ...
  if (draw_checkbox(widget, "Show Gradient", app.show_gradient)) {
    ...
    if (app.show_gradient) {
      if (app.curr_bad_t < app.bad_t.size()) {
        auto t       = app.bad_t[app.curr_bad_t];
        ...
        //   std::tie(app.karcher_grads, app.karcher_fields) =
        //       compute_karcher_grad(*app.mesh.flipout, vertices,
        //       app.mesh.solver,
        //           app.mesh.triangles, app.mesh.positions, app.mesh.normals,
        //           app.mesh.Grad);
  ...
  //   vector<vector<int>>        vertices(control_polygon.size(),
  //   vector<int>(4)); vector<vector<vec3f>>
  //   flipout_pos(control_polygon.size()); vector<vector<mesh_point>> points(
  ...
  //     app.WA_curve_shape = add_path_shape(
  //         app, app.WA_curve, 0.003f * app.curve_size, vec3f{0, 1, 0}, true);
  ...
  //         add_control_point(app, spline, control_polygon[j][i]);
  //       }

  //       add_points_shape(app,
  //           {control_polygon[j][0], control_polygon[j][1],
  //               control_polygon[j][2], control_polygon[j][3]},
  //           0.003f * app.line_size, {0, 0, 1});
  //       auto L0 = my_compute_geodesic_path(
  //           app.mesh, control_polygon[j][0], control_polygon[j][1]);
  //       auto L1 = my_compute_geodesic_path(
  //           app.mesh, control_polygon[j][1], control_polygon[j][2]);
  //       auto L2 = my_compute_geodesic_path(
  //           app.mesh, control_polygon[j][2], control_polygon[j][3]);
  ...
  //       update_curve_shape(spline, app.good_samples, app.bad_samples, j,
  //           app.mesh, app._bezier_params, app.bad_t, app.bad_points, false);
  //     }
  //   }
  ...
  //   //           *app.mesh.flipout, vertices[j],
  //   //           app._bezier_params.subdivisions);
  //   //       auto curr_pos = generate_polyline_from_positions(app.mesh,
  //   //           flipout::path_positions(bezier.get()), points[j][0].face);

  //   //       add_path_shape(
  //   //           app, curr_pos, 0.005f * app.curve_size, vec3f{1, 1, 0});
  //   //       if (app.show_WA_samples)
  //   //         app.WA_samples_shape = add_points_shape(
  ...
  // continue_line(widget);
  // if (draw_checkbox(widget, "WA samples", app.show_WA_samples)) {
  ...
  // }
  draw_separator(widget);
  if (draw_combobox(widget, "algorithm", (int&)app._bezier_params.algorithm,
          spline_algorithm_names)) {
    update_all_splines(app);
  }
  draw_slider(widget, "subdivisions", app._bezier_params.subdivisions, 1, 10);
  draw_slider(widget, "precision", app._bezier_params.precision, 0, 1.f);
  // draw_slider(widget, "Bad t", app.curr_bad_t, 0, app.bad_t.size());
  ...
  if (draw_slider(widget, "t parameter", app.input().parameter_t, 0.0f, 1.0f)) {
    ...
    if (!app.weighted_average) {
      ...
      //     app.mesh, polygon, app._bezier_params, 20);
    }
  }
  ...
  if (draw_button(widget, "Show Algorithm")) {
    ...
    if (app._bezier_params.algorithm == spline_algorithm::subdivision_uniform) {
      // control polygon
  ...
  draw_checkbox(widget, "parallel", app._bezier_params.parallel);
  draw_checkbox(widget, "show edges", app.show_edges);
  // app.shade_params.faceted = app.show_edges;
  draw_checkbox(widget, "show points", app.show_points);
  ...
  if (draw_slider(widget, "curve size", app.curve_size, 0, 1.0f)) {
    for (auto& spline : app.splines()) {
      for (auto& curve : spline.curves) {
        ...
        update_path_shape(
            curve.shape, app.mesh, curve.positions, 0.005f * app.curve_size);
        set_polyline_shape(
            curve.tangents[0].shape, curve.tangents[0].positions);
        set_polyline_shape(
            curve.tangents[1].shape, curve.tangents[1].positions);
  ...
  // draw_slider(
  //     widget, "avg length scale factor", app.avg_edge_length_factor,
  //     1, 50.0f);
  ...
  //         }
  //         update_path_shape(
  //             curve.shape, app.mesh, curve.positions, 0.003f *
  //             app.curve_size);
  //         set_polyline_shape(
  //             curve.tangents[0].shape, curve.tangents[0].positions);
  //         set_polyline_shape(
    ...
    if (app.added_paths.size() != 0) {
      ...
      switch (app.level) {
        ...
        case 2: {
          ...
          //       auto& curve = spline.curves[i];
          //       set_uniform(
          //           program, "color", vec3f{app.red, app.green, app.blue});
          //       draw_shape(curve.shape->shape);
          //     }
          //   }
          // } break;
...
/*
istream& read_point(istream& str, vector<vec3f>& points){
  std::string line;    
  float coord[3];
  for(int i = 0; i < 3; i++) {
    if(!std::getline(str, line)) return str;
    coord[i] = std::stof(line);
  }
  vec3f point = vec3f{coord[0], coord[1], coord[2]};
  points.push_back(point);
  return str;
}
*/

istream& read_point_bar(istream& str, vector<mesh_point>& points){
  std::string line;
  if(!std::getline(str, line)) return str; //Empty
  int face;    
  float x, y;
  //Read face
  face = std::stoi(line);
...
int send_polyline(int ListenSocket, vector<mesh_point>& poly){
  int n = poly.size();
  std::string ret =  std::to_string(n) + "\n";
  send(ListenSocket, ret.c_str(), ret.length(), 0);
  ret = "";
  for(int i = 0; i < n; i++){
    ret += std::to_string(poly[i].face) + " " + std::to_string(poly[i].uv.x) + " " + std::to_string(poly[i].uv.y) + "\n"; 
  }
  send(ListenSocket, ret.c_str(), ret.length(), 0);
...
int listen_blender(int ListenSocket, App& app){
    int ClientSocket;
    ...
    std::string line; //For reading input
    vector<mesh_point> tmp; //Storing input points
    ...
    do {
        ...
        if (iResult > 0) {
          ...
          if(recvbuf[0] == 'a') {
              ...
              std::cout<<"connention closed...\n";
              return 0;
          }
          //Extend curve request
          if(recvbuf[0] == 'n') {
            std::istringstream str(std::string(recvbuf, iResult));
            std::getline(str, line); //Command line 'n', discard
            while(str) read_point_bar(str, tmp);
            //Compute tangent
            auto path = compute_geodesic_path(app.mesh, tmp[1], tmp[0]);
            auto positions = path_positions(app.mesh, path);
            float tan_len = path_length(positions);
            auto tangent = continue_path(app.mesh, path, -tan_len);
            //Send new control point (Tangent extension)
            line =  std::to_string(tangent.end.face) + " " + std::to_string(tangent.end.uv.x) + " " + std::to_string(tangent.end.uv.y) + "\n";
            send(ClientSocket, line.c_str(), line.length(), 0);
          //Rotate tangent
          }else if(recvbuf[0] == 'r'){
            geodesic_path result;
            int end = 0;
            if(recvbuf[1] == '1') end = 1;
            std::istringstream str(std::string(recvbuf, iResult));
            std::getline(str, line); //Command line 'r', discard
            while(str) read_point_bar(str, tmp);
            //Compute path and send
            auto p1 = compute_geodesic_path(app.mesh, tmp[1], tmp[0]);
            auto p2 = compute_geodesic_path(app.mesh, tmp[1], tmp[2]);
            if(end == 0){
              auto positions = path_positions(app.mesh, p1);
              float tan_len = path_length(positions);
              result = continue_path(app.mesh, p2, -tan_len);
            }
            else{
              auto positions = path_positions(app.mesh, p2);
              float tan_len = path_length(positions);
              result = continue_path(app.mesh, p1, -tan_len);
            };
            line =  std::to_string(result.end.face) + " " + std::to_string(result.end.uv.x) + " " + std::to_string(result.end.uv.y) + "\n";
            send(ClientSocket, line.c_str(), line.length(), 0);
          }
          //Line for control polygon
          else if(recvbuf[0] == 'l'){
            std::istringstream str(std::string(recvbuf, iResult));
            std::getline(str, line); //Command line 'l', discard
            while(str) read_point_bar(str, tmp);
            //Compute path and send
            auto path = compute_geodesic_path(app.mesh, tmp[0], tmp[1]);
            auto res = path_positions_meshpoint(app.mesh, path);  
            send_polyline(ClientSocket, res);
          }
          //Eval point for split
           else if(recvbuf[0] == 'p'){
            //vector<mesh_point> tmp;
            std::istringstream str(std::string(recvbuf, iResult));
            std::getline(str, line); //Command line 'l', discard
            std::getline(str, line);
            float t0 = std::stof(line);
            while(str) read_point_bar(str, tmp);
            bezier_segment polygon = bezier_segment{};
            for (int i = 0; i < tmp.size(); ++i) polygon[i] = tmp[i];
            mesh_point result = eval_bezier_point(app.mesh, polygon, t0, 0.f, 1.0f);
            line =  std::to_string(result.face) + " " + std::to_string(result.uv.x) + " " + std::to_string(result.uv.y) + "\n";
            send(ClientSocket, line.c_str(), line.length(), 0);
           }
          //split polygon
          else if(recvbuf[0] == 's'){
            ...
            for (int i = 0; i < tmp.size(); ++i) {
              polygon[i] = tmp[i];
              std::cout<<"Polygon  f: " << tmp[i].face << " u: " << tmp[i].uv.x << " v: " << tmp[i].uv.y<< "\n";
            }
            
            auto res = insert_point( app.mesh, polygon, t0);
            vector<mesh_point> to_send; //Storing ouput points
            //Print
            //for(int i = 0; i<4; i++) std::cout<<"Segment 1  f: " << res[0][i].face << " u: " << res[0][i].uv.x << " v: " << res[0][i].uv.y<< "\n";
            //for(int i = 0; i<4; i++) std::cout<<"Segment 2  f: " << res[1][i].face << " u: " << res[1][i].uv.x << " v: " << res[1][i].uv.y <<"\n";
            //End print
            for(int i = 0; i<4; i++) to_send.push_back( res[0][i] );
            for(int i = 1; i<4; i++) to_send.push_back( res[1][i] );

            send_polyline(ClientSocket, to_send);
          }
          //Params
          else if(recvbuf[0] == 'o'){
            if(recvbuf[1] == 'd' ) app._bezier_params.algorithm = spline_algorithm::de_casteljau_uniform;
            else  app._bezier_params.algorithm = spline_algorithm::subdivision_uniform;
            //Set subdivisions
            std::istringstream str(std::string(recvbuf, iResult));  
            std::getline(str, line); //Command line 'o', discard
            std::getline(str, line); //number of subdivision
            int n = std::stoi(line);
            app._bezier_params.subdivisions = n;
          }
          //Calculate curve from scratch
          else{
            std::istringstream str(std::string(recvbuf, iResult));
            while(str) read_point_bar(str, tmp); 
            //Calculate curve
            ...
            auto points = bezier_uniform(app.mesh, polygon, app._bezier_params);
            auto res = make_polyline_positions_meshpoints(app.mesh, points);
            // Send curve
            send_polyline(ClientSocket, res);
          }
...
int blender_connection(App& app) {

    int ListenSocket, iResult;
    struct sockaddr_in address;
    int opt = 1;
    ...
    t1 = std::thread(listen_blender, ListenSocket, std::ref(app));
    return 0;
}
...
int main(int num_args, const char* args[]) {
  ...
  // *app.mesh.flipout = make_flipout_mesh(app.mesh.triangles,
  // app.mesh.positions);
