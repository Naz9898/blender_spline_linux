...
void update_curve_shape(Gui_Spline& spline, vector<vec3f>& good_samples,
    vector<vec3f>& bad_samples, int curve_id, const bezier_mesh& mesh,
    const bezier_params& params, vector<float>& bad_t,
    vector<mesh_point>& points, const bool jumps, const bool quadric,
  ...
  points.clear();
  if (params.algorithm == spline_algorithm::de_casteljau_uniform) {
    points = (quadric) ? bezier_uniform(mesh, quadric_polygon, params)
                       : bezier_uniform(mesh, polygon, params);
  } else if (params.algorithm == spline_algorithm::de_casteljau_adaptive) {
    points = bezier_adaptive(mesh, polygon, params);
  } else if (params.algorithm == spline_algorithm::subdivision_uniform) {
    points = (quadric) ? spline_subdivision_uniform(
    ...
    vector<int> badones;
    points = de_casteljau_classic(mesh, polygon, params, badones, jumps);
    bool prev_is_good = true;
    for (auto i = 1; i < points.size(); ++i) {
      auto curr_is_good =
          (std::find(badones.begin(), badones.end(), i) == badones.end());
      if (curr_is_good && prev_is_good) {
        auto pos = make_polyline_positions(mesh, {points[i - 1], points[i]});
        curve.positions.insert(curve.positions.end(), pos.begin(), pos.end());
        prev_is_good = true;
      } else if (!curr_is_good) {
        prev_is_good = false;
        // auto n       = triangle_normal(
        //     mesh.positions[mesh.triangles[points[i - 1].face].x],
        //     mesh.positions[mesh.triangles[points[i - 1].face].y],
        //     mesh.positions[mesh.triangles[points[i - 1].face].z]);
        // auto curr_pos = eval_position(mesh, points[i - 1]) - 0.01 * n;
        // curve.positions.push_back(curr_pos);
        // n = triangle_normal(mesh.positions[mesh.triangles[points[i].face].x],
        //     mesh.positions[mesh.triangles[points[i].face].y],
        //     mesh.positions[mesh.triangles[points[i].face].z]);
        auto curr_pos = eval_position(mesh, points[i]);
        ...
        prev_is_good = true;
        // auto n       = triangle_normal(
        //     mesh.positions[mesh.triangles[points[i].face].x],
        //     mesh.positions[mesh.triangles[points[i].face].y],
        //     mesh.positions[mesh.triangles[points[i].face].z]);
        auto curr_pos = eval_position(mesh, points[i]);
        curve.positions.push_back(curr_pos);
    ...
    // points = bezier_karcher_test(mesh, polygon, params.subdivisions,
    // badones); curve.positions.clear(); auto vertices = vector<int>(4); for
    // (int i = 0; i < 4; i++) {
    //   vertices[i] = vert_from_point(
    //       mesh.triangles, polygon[i]);  // mesh.triangles[polygon[i].face].x;
    // }
    // points = karcher_average(mesh,
    //     {polygon[0], polygon[1], polygon[2], polygon[3]},
    //     params.subdivisions, use_vector_heat, bad_t, badones);
    // good_samples.push_back(
    //     mesh.positions[vert_from_point(mesh.triangles, polygon[0])]);
    // bool prev_is_good = true;
    ...
    //     good_samples.push_back(eval_position(mesh, points[i]));
    //   } else if (!curr_is_good) {
    //     prev_is_good = false;
    //     bad_samples.push_back(eval_position(mesh, points[i]));
    ...
    // }
    // // if (prev_is_good) {
    // auto pos = make_polyline_positions(mesh, {points.back(), polygon[3]});
    // curve.positions.insert(curve.positions.end(), pos.begin(), pos.end());
    // // } else
    // //   curve.positions.push_back(eval_position(mesh, polygon[3]));
    // printf("num points: %d\n", points.size());
  }
...
void init_gpu(App& app, bool envlight) {
  ...
  app._shade_params.hide_environment = true;
  app._shade_params.exposure         = -0.5;
  app._shade_params.background       = {1, 1, 1, 1};
  app._shade_params.lighting         = envlight ? shade_lighting_type::envlight
...
void update_path_shape(shade_shape* shape, const bezier_mesh& mesh,
    const vector<vec3f>& positions, float radius, const float& treshold) {
  // if (thin) {
  //   set_positions(shape, positions);
  //   shape->shape->elements = ogl_element_type::line_strip;
  //   set_instances(shape, {});
  ...
  for (int i = 0; i < positions.size() - 1; i++) {
    auto from = positions[i];
    auto to   = positions[i + 1];
    if (from == to || length(to - from) >= treshold) continue;
...
void export_scene(App& app, const string& filename, const bool export_edges,
    ...
    for (auto tr : app.mesh.triangles) {
      for (int k = 0; k < 3; ++k) {
        ...
        polyline_to_cylinders(quads, pos, normals, tex,
            {app.mesh.positions[a], app.mesh.positions[b]}, 4, 0.0004);
  ...
  for (auto i = 0; i < app.splines().size(); ++i) {
    ...
    for (auto curve : spline.curves) {
      ...
      polyline_to_cylinders(
          quads, pos, normals, tex, curve.positions, 4, 0.0025);
    ...
    for (auto j = 0; j < spline.control_points.size() - 1; ++j) {
      auto path = compute_geodesic_path(
          app.mesh, spline.control_points[j], spline.control_points[j + 1]);
      auto quads   = vector<vec4i>{};
      auto pos     = vector<vec3f>{};
      auto normals = vector<vec3f>{};
      auto tex     = vector<vec2f>{};
      ...
      polyline_to_cylinders(
          quads, pos, normals, tex, path_positions(app.mesh, path), 4, 0.0020);

      auto tri = quads_to_triangles(quads);
      merge_triangles(final_tri, tri, size);
      final_pos.insert(final_pos.end(), pos.begin(), pos.end());
      final_normals.insert(final_normals.end(), normals.begin(), normals.end());
      auto curr_colors = vector<vec3f>(pos.size(), {0, 0, 1});
      final_colors.insert(
          final_colors.end(), curr_colors.begin(), curr_colors.end());
      size += (int)pos.size();
      // if (i > 0 && j > 0) continue;
  ...
  points_to_spheres(quads, pos, normals, tex, point_pos, 4, 0.0030);
