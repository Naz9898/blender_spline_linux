...
void read_blender_obj(const string& filename, vector<vec3f>& positions, vector<vec3i>& triangles){
      //std::cout << "\nReading blender mesh" << std::endl;
      ...
      while (std::getline(file, str))
      {
          ...
          string word;
          while (std::getline(sstream, word, ' ')) tokens.push_back(word);
          if(tokens[0] == "v"){
            vec3f point{std::stof(tokens[1]), std::stof(tokens[2]), std::stof(tokens[3])};
            positions.push_back(point);
          }
          if(tokens[0] == "f"){
            vec3i face{std::stoi(tokens[1])-1, std::stoi(tokens[2])-1, std::stoi(tokens[3])-1};
            triangles.push_back(face);
          }
      }
      //Print obj
      /*
      std::cout << "Current points "<<positions.size() << std::endl;
      for(int i = 0; i < positions.size(); i++){
        std::cout << positions[i].x << " " << positions[i].y << " " << positions[i].z << " " << std::endl;
      }
      std::cout << "Current triangles "<<triangles.size() << std::endl;
      for(int i = 0; i < triangles.size(); i++){
        std::cout << triangles[i].x << " " << triangles[i].y << " " << triangles[i].z << " " << std::endl;
      }*/
}
...
bool load_mesh(const string& filename, bezier_mesh& mesh, string& error) {
  ...
  if (ext == ".stl") {
    ...
    read_blender_obj(filename, mesh.positions, mesh.triangles);
  }
  if (quads.size()) {
