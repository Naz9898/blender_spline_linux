...
inline vector<mesh_point> path_positions_meshpoint(
    const bezier_mesh& mesh, const geodesic_path& path) {
  return path_positions_meshpoint(path, mesh.triangles, mesh.positions, mesh.adjacencies);
}
...
inline vector<mesh_point> make_polyline_positions_meshpoints(
    const bezier_mesh& mesh, const vector<mesh_point>& points) {
  auto result = vector<mesh_point>();  // TODO: reserve what?
  for (int i = 0; i < points.size() - 1; i++) {
    auto a    = points[i];
    auto b    = points[i + 1];
    auto path = compute_geodesic_path(mesh, a, b);
    append(result, path_positions_meshpoint(mesh, path));
  }
  return result;
}
