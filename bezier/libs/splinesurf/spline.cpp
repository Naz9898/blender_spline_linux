...
std::array<bezier_segment, 2> insert_point(
  ...
  // left part
  {
    ...
    auto path = straightest_path(mesh, P, Pp2_dir, delta_len);
    auto Pp2  = path.end;

    auto Pp1 = geodesic_lerp(
        mesh, tree.nodes[0].points[0], tree.nodes[0].points[1], t0);
  ...
  // right part
  {
    ...
    auto path     = straightest_path(mesh, P, Pp1_dir, Pp1_len / t0_local);
    auto Pp1      = path.end;
    auto Pp2      = geodesic_lerp(
        mesh, tree.nodes[0].points[2], tree.nodes[0].points[3], t_end);
