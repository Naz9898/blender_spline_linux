...
//         printf("%.*s", string.count, buffer.buffer[string.start]);
//     }
// }
...
inline string convert_nanoseconds(int64_t t, log_unit unit) {
  ...
  if (unit == log_unit::microsecond) {
    ...
    return format("$.$$$ms", _s, n2, n1, n0);
  }
  if (unit == log_unit::millisecond) {
    auto _0 = next_digit(_s);
    auto _1 = next_digit(_s);
    auto _2 = next_digit(_s);
    return format("$.$$$ms", ms, _2, _1, _0);
  }
