...
vector<mesh_point> path_positions_meshpoint(const geodesic_path &path,
    const vector<vec3i> &triangles, const vector<vec3f> &positions,
    const vector<vec3i> &adjacencies) {
  auto result = vector<mesh_point>(path.lerps.size() + 2);
  result[0] = path.start;
  for (auto i = 0; i < path.lerps.size(); i++) {
    auto k = find_in_vec(adjacencies[path.strip[i]], path.strip[i+1]);
    auto bary = zero3f;
    auto x        = path.lerps[i];
    bary[k] = 1-x;
    bary[(k+1)%3] = x;
    result[i + 1] = mesh_point{path.strip[i], {bary.y, bary.z}};
  }
